...
RSpec.describe 'Transaction', type: :feature do
  describe 'transaction' do
    before(:each) do
      @user = User.create! name: 'jones', password: 'angel2000', email: 'jones@gmail.com', role: 'admin'
      visit new_user_session_path
      fill_in 'Email', with: 'jones@gmail.com'
      fill_in 'Password', with: 'angel2000'
      click_button 'Log in'
      @category = Category.create(name: 'one', icon: 'grams')
      @transaction = Entity.create(name: 'test', amount: 9, category_id: @category.id, author_id: @user.id)
